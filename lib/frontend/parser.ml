...
let parse_int tok par =
    ...
    | None ->
        let par' = report_error "could not parse integer" tok par in
...
let parse_float tok par =
    ...
    | None ->
        let par' = report_error "could not parse float" tok par in
        (ErrorExpr tok.span, par')
let parse_char tok par =
    match char_of_string (lexeme tok par) with
    | Some value -> (Literal (CharLiteral value), par)
    | None ->
        let par' = report_error "could not parse character" tok par in
        (ErrorExpr tok.span, par')

let parse_string tok par =
    let value = lexeme tok par in 
    (Literal (StringLiteral value), par)

let parse_bool value par =
    (Literal (BoolLiteral value), par)

let parse_identifier tok par =
    let value = lexeme tok par in 
    (Ast.Identifier value, par)
...
and parse_grouping par =
    ...
    | _          -> let par'' = report_error "expected ')' to close grouping" tok par' in (Grouping expr, par'') (* still have a valid grouping, just not closed properly *)
